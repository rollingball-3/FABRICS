...
class _CS63TesolloGripperAttractorBase(CS63TesolloBaseFabric):
    """
    Base class for gripper-attractor variants (joint space).
    """
    ...
    def _add_gripper_attractor_task(self):
        ...
        self.add_taskmap(
            self._GRIPPER_ATTRACTOR_TASKMAP_NAME, LinearMap(P_full, self.device), graph_capturable=self.graph_capturable
        )
        self.add_fabric(
            self._GRIPPER_ATTRACTOR_TASKMAP_NAME,
            "gripper_attractor",
            Attractor(True, self.fabric_params["gripper_attractor"], self.device, graph_capturable=self.graph_capturable),
        )
    ...
    def construct_fabric(self):
        ...
        if self.use_gripper_attractor:
            self._add_gripper_attractor_task()
...
class CS63TesolloGripperLatentAttractorFabric(_CS63TesolloGripperAttractorBase):
    """
    Latent-space attractor variant (e.g. synergy).
    """
