...
from fabrics_sim.fabrics.cs63_tesollo_fabric import CS63TesolloForceFabric
...
"""
Example usage:
python cs63_tesollo_force_example.py --batch_size=2 --cuda_graph
"""

def main():
    parser = argparse.ArgumentParser(description='CS63-Tesollo Force fabric example.')
    ...
    # Starting joint state (arm 6 + gripper 12)
    q_start = torch.zeros(18, device=device)
    q_start[1] = -1.57 # shoulder lift
    q_start[2] = 1.0 # elbow
    
    fabric = CS63TesolloForceFabric(args.batch_size, device, timestep, default_joint_config=q_start)
    ...
    # Force targets: (b, 3, 3)
    finger_forces = torch.zeros(args.batch_size, 3, 3, device=device)
    ...
    if args.cuda_graph:
        inputs = [finger_forces, palm_pos, palm_rot, q.detach(), qd.detach(), 
    ...
    for i in range(200):
        if i % 50 == 0:
            finger_forces.uniform_(-1.0, 1.0)
        ...
        if args.cuda_graph:
            ...
            fabric.set_features(finger_forces, palm_pos, palm_rot, q.detach(), qd.detach(), 
                                object_ids, object_indicator, damping_gain)
            q, qd, qdd = integrator.step(q.detach(), qd.detach(), qdd.detach(), timestep)

        if i % 20 == 0:
            print(f"Step {i}, q[0]: {q[0, :3].cpu().numpy()}")
